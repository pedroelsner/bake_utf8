<?php
/**
 * Arquivo que executa o comando 'bake_utf8 view_utf8'
 *
 ...
 */
    function execute()
    {
        ...
        $this->ControllerUtf8Name = $this->_controllerName($this->args[0]);
        $this->ControllerUtf8Path = $this->_controllerPath($this->ControllerUtf8Name);
 ...
 */
    ...
    {
        $methods =  array_diff(
            array_map('strtolower', get_class_methods($this->ControllerUtf8Name . 'Controller')),
        ...
        foreach ($methods as $i => $method)
        {
            ...
            if ($method[0] === '_' || $method == strtolower($this->ControllerUtf8Name . 'Controller'))
            {
                unset($methods[$i]);
            }
        }
 ...
 * All
 *
 * Respons�vel pelo comando 'bake_utf8 view_utf8 all'
 *
 * @access public
 */
    function all()
    {
        $this->ControllerUtf8->interactive = false;
        $tables = $this->ControllerUtf8->listAll($this->connection, false);
        ...
        foreach ($tables as $table)
        {
            ...
            $this->ControllerUtf8Name = $this->_controllerName($model);
            $this->ControllerUtf8Path = Inflector::underscore($this->ControllerUtf8Name);
 ...
 */
    function __interactive()
    {
        ...
        $this->DbConfig->interactive = $this->ControllerUtf8->interactive = $this->interactive = true;
        ...
        $this->ControllerUtf8->connection = $this->connection;
        $this->ControllerUtf8Name = $this->ControllerUtf8->getName();

        $this->ControllerUtf8Path = strtolower(Inflector::underscore($this->ControllerUtf8Name));

        $prompt = sprintf(__("Would you like bake to build your views interactively?\nWarning: Choosing no will overwrite %s views if it exist.", true),  $this->ControllerUtf8Name);
 ...
 */
    ...
    {
        if (!$this->ControllerUtf8Name)
        {
            $this->err(__('Controller not found', true));
        }

        $import = $this->ControllerUtf8Name;
        if ($this->plugin)
        {
            $import = $this->plugin . '.' . $this->ControllerUtf8Name;
        }

        if (!App::import('Controller', $import))
        {
            $file = $this->ControllerUtf8Path . '_controller.php';
        ...
        $controllerClassName = $this->ControllerUtf8Name . 'Controller';
        ...
        if ($modelObj)
        {
            ...
            $singularHumanName = $this->_singularHumanName($this->ControllerUtf8Name);
        ...
        else
        {
            ...
            $singularVar = Inflector::variable(Inflector::singularize($this->ControllerUtf8Name));
            $singularHumanName = $this->_singularHumanName($this->ControllerUtf8Name);
            $fields = $schema = $associations = array();
        }
        $pluralVar = Inflector::variable($this->ControllerUtf8Name);
        $pluralHumanName = $this->_pluralHumanName($this->ControllerUtf8Name);
 ...
 */
    function customAction()
    {
        ...
        $this->hr();
        $this->out(sprintf(__('Controller Name: %s', true), $this->ControllerUtf8Name));
        $this->out(sprintf(__('Action Name:     %s', true), $action));
        $this->out(sprintf(__('Path:            %s', true), $this->params['app'] . DS . $this->ControllerUtf8Path . DS . Inflector::underscore($action) . ".ctp"));
        $this->hr();
 ...
 */
    function bake($action, $content = '')
    {
        ...
        $path = $this->getPath();
        $filename = $path . $this->ControllerUtf8Path . DS . Inflector::underscore($action) . '.ctp';
 ...
 * Help
 *
 * Responde o comando 'bake_utf8 view_utf8 help'
 *
 * @access public
 */
    function help() {
